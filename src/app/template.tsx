...
const Template = ({children}:{children: React.ReactNode}) => {
    ...
    return (
        <div className="overflow-hidden">
            ...
            <div id="LinkText" className="text-white text-center z-50 fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2">{`${pathname === '/'?'Glad you\'re here. Welcome!':pathname === '/about'?'About Me':pathname === '/cv'?'Curriculum Vitae':pathname === '/contact'?'Let\'s Connect':pathname}`}</div>
            {children}
        </div>
    );
}
